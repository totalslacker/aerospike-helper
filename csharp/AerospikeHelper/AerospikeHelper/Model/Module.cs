...
namespace Aerospike.Helper.Model
{
	public class Module
	{
		protected static readonly ILog log = LogManager.GetLogger (typeof(Module));
		private String name;
		...
		private void KvPut(String kv, Dictionary<String, String> map){
			...
			if (map.ContainsKey(kvParts[0]))
				map[kvParts[0]] = kvParts[1];
			else
		...
		public void DetailInfo(String info){
			...
			this.source = "";
			//TODO decode the module and extract the source
//			try {
//				String code = values["content"];
//				code = code.Substring(0, code.Length-3);
//				log.Info (code);
//				byte[] decoded = Convert.FromBase64String(code);
//				Source = System.Text.Encoding.UTF8.GetString(decoded);
//			} catch (Exception ex) {
//				log.Error (string.Format ("{0} - {1} {2}", this.GetType (), ex.GetType (), ex.Message));
